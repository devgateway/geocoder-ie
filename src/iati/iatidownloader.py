...
def process_activity(act_doc, org):
    ...
    if len(act_doc.findall("document-link/category[@code='A02']") + act_doc.findall(
        ...
        region_code = int(act_doc.find('recipient-region').get('code')) if act_doc.find(
            'recipient-region') != None else None
        country_code = act_doc.find('recipient-country').get('code') if act_doc.find(
            'recipient-country') != None else None
        ...
        path = create_folder('%s/%s' % (PATH_TO_PROJECTS,organisations[org]), folder)

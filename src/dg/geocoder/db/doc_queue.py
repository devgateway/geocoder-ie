...
def delete_all_docs_from_queue():
    conn = open()
    sql_1 = "DELETE FROM EXTRACT WHERE QUEUE_ID IN (SELECT ID FROM QUEUE );"
    sql_2 = "DELETE FROM location_activity_descriptions WHERE location_id IN (SELECT ID FROM LOCATION WHERE QUEUE_ID IN (SELECT ID FROM QUEUE )) ;"
    sql_3 = "DELETE FROM location_descriptions WHERE location_id IN (SELECT ID FROM LOCATION WHERE QUEUE_ID IN (SELECT ID FROM QUEUE )) ;"
    sql_4 = "DELETE FROM location_identifier WHERE location_id IN (SELECT ID FROM LOCATION WHERE QUEUE_ID IN (SELECT ID FROM QUEUE )) ;"
    sql_5 = "DELETE FROM location_names WHERE location_id IN (SELECT ID FROM LOCATION WHERE QUEUE_ID IN (SELECT ID FROM QUEUE )) ;"
    sql_6 = "DELETE FROM administrative WHERE location_id IN (SELECT ID FROM LOCATION WHERE QUEUE_ID IN (SELECT ID FROM QUEUE )) ;"
    sql_7 = "DELETE FROM LOCATION WHERE QUEUE_ID IN (SELECT ID FROM QUEUE );"
    sql_8 = "DELETE FROM QUEUE;"

    # WHERE STATE IN ('PENDING','ERROR','PROCESSING')
    cur = conn.cursor()
    cur.execute(sql_1)
    cur.execute(sql_2)
    cur.execute(sql_3)
